...
void SendData(int Command, unsigned int yValue,unsigned int xValue){
   ...
   if(yLSB==0){ yLSB=1; zeroByte=zeroByte+1;}        // 1bit目を1に
   if(yMSB==0){ yMSB=1; zeroByte=zeroByte+2;}        // 2bit目を1に
   if(xLSB==0){ xLSB=1; zeroByte=zeroByte+4;}        // 3bit目を1に
   if(xMSB==0){ xMSB=1; zeroByte=zeroByte+8;}        // 4bit目を1に
